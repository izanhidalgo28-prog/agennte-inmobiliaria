...
module.exports = async function handler(req, res) {
  ...
  if (conv.modo === 'valoracion') {
    if (conv.paso === 11) {
      ...
      conv.paso = 12;
      await guardarEstado();
      return responder(L.valorTipo);
    }
    if (conv.paso === 12) {
      conv.datos.zonaValor = userMessage;
      conv.paso = 13;
      await guardarEstado();
      return responder(L.valorMetros);
    }
    if (conv.paso === 13) {
      conv.datos.metros = userMessage;
      conv.paso = 14;
      await guardarEstado();
      return responder(L.valorHabitaciones);
    }
    if (conv.paso === 14) {
      conv.datos.habitaciones = userMessage;
      conv.paso = 15;
      await guardarEstado();
      return responder(L.valorBanos);
    }
    if (conv.paso === 15) {
      conv.datos.banos = userMessage;
      conv.paso = 16;
      await guardarEstado();
      return responder(L.valorEstado);
    }
    if (conv.paso === 16) {
      ...
      try {
        ...
        return responder(L.errorValoracion);
      }
    }
  }
  ...
  if (conv.modo === 'consulta') {
    ...
    if (msg.includes('agente') || msg.includes('contactar') || msg.includes('llamar') || msg.includes('agent') || msg.includes('contact') || msg.includes('makler') || msg.includes('kontakt')) {
      return responder(L.agenteContacto);
    }
    if (msg === 'inicio' || msg === 'start' || msg === 'reset') {
      await redis.del(from);
      return responder(L.reinicio);
    }
    try {
      const respuesta = await consultarIA(userMessage);
      return responder(respuesta);
    } catch(e) {
      return responder(L.errorIA);
    }
  }
  ...
  if (conv.leadGuardado && conv.modo !== 'consulta') {
    conv.modo = 'consulta';
    await guardarEstado();
    return responder(L.holaDeNuevo);
  }

  if (conv.paso === 0) {
    conv.paso = 1;
    await guardarEstado();
    return responder(L.bienvenida);
  }

  if (conv.paso === 1) {
    const msg = userMessage.toLowerCase();
    if (msg.includes('valor') || msg.includes('2') || msg.includes('value') || msg.includes('bewert')) {
      conv.modo = 'valoracion';
      conv.paso = 11;
      await guardarEstado();
      return responder(L.valorarInicio);
    }
    if (msg.includes('vender') || msg.includes('sell') || msg.includes('verkauf')) {
      conv.datos.busca = 'Vender propiedad';
      conv.paso = 2;
      await guardarEstado();
      return responder(L.venderZona);
    }
    conv.datos.busca = userMessage;
    conv.paso = 2;
    await guardarEstado();
    return responder(L.zonaPregunta);
  }

  if (conv.paso === 2) {
    if (!esTextoValido(userMessage)) {
      return responder(L.zonaInvalida);
    }
    conv.datos.zona = userMessage;
    conv.paso = 3;
    await guardarEstado();
    return responder(L.presupuestoPregunta);
  }

  if (conv.paso === 3) {
    const limpio = userMessage.replace(/[€.\s]/g,'');
    if (!esNumeroValido(limpio)) {
      return responder(L.presupuestoInvalido);
    }
    conv.datos.presupuesto = limpio + '€';
    conv.paso = 4;
    await guardarEstado();
    return responder(L.caracteristicasPregunta);
  }

  if (conv.paso === 4) {
    if (!esTextoValido(userMessage)) {
      return responder(L.caracteristicasInvalidas);
    }
    conv.datos.caracteristicas = userMessage;
    conv.paso = 5;
    await guardarEstado();
    return responder(L.nombrePregunta);
  }

  if (conv.paso === 5) {
    if (!esTextoValido(userMessage) || /\d/.test(userMessage)) {
      return responder(L.nombreInvalido);
    }
    conv.datos.nombre = userMessage;
    conv.paso = 6;
    await guardarEstado();
    return responder(L.telefonoPregunta);
  }

  if (conv.paso === 6) {
    if (!esTelefonoValido(userMessage)) {
      return responder(L.telefonoInvalido);
    }
